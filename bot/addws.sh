...
sed -i '/#LUNATIX-VMESS#$/a\### '"$user $exp"'\
...
if [[ ${c} != "0" ]]; then
  ...
  echo "${d}" >/etc/lunatic/vmess/usage/${user}  
fi

DATADB=$(cat /etc/lunatic/vmess/.vmess.db | grep "^#vm#" | grep -w "${user}" | awk '{print $2}')
if [[ "${DATADB}" != '' ]]; then
  sed -i "/\b${user}\b/d" /etc/lunatic/vmess/.vmess.db
fi
status="UNLOCKED"
echo "### ${user} ${expe} ${status} ${uuid} ${Quota}" >>/etc/lunatic/vmess/.vmess.db
