...
echo ""
read -p "  Limit ip    : " ip
echo $ip > /etc/lunatic/trojan/ip/${user}
...
sed -i '/#LUNATIX-TROJAN#$/a\#! '"$user $exp"'\
...
if [[ ${c} != "0" ]]; then
  ...
  echo "${d}" >/etc/lunatic/trojan/usage/${user}  
fi
DATADB=$(cat /etc/lunatic/trojan/.trojan.db | grep "^#!" | grep -w "${user}" | awk '{print $2}')
if [[ "${DATADB}" != '' ]]; then
  sed -i "/\b${user}\b/d" /etc/lunatic/trojan/.trojan.db
fi
echo "#! ${user} ${exp} ${uuid} ${Quota}" >>/etc/lunatic/trojan/.trojan.db
clear
echo -e "\033[1;36m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
